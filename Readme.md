...
Time taken: 12345 ms
```
To evaluate the accuracy of the CUDA stream-optimized digit recognition process on the MNIST dataset, we provide a Python script, `accuracy.py`. This script compares the predicted digit classifications against the true labels for each image in the batch-processed dataset, calculating the overall accuracy of the model.

### Accuracy Evaluation Script for assignment2_subtask4.cu : `accuracy.py`

#### Features:

- **Label Reading**: Reads the true labels from text files in the batch labels directory.
- **Prediction Reading**: Extracts the top predictions from the probability files generated by the CUDA stream processing.
- **Accuracy Calculation**: Computes the overall accuracy of the digit recognition process by comparing predictions against true labels.

#### Running the Script:

Execute the script in an environment with Python installed. From the terminal, navigate to the directory containing `accuracy.py` and run:
...
The script outputs the accuracy of the digit recognition process, displayed as a percentage:

```
Accuracy: 98.76%
```

This indicates the percentage of MNIST images correctly classified by the model.
